...
function addDuration(config, row) {
  ...
  row['Duration'] = durationParser(row[config.durationColumn]);
  return row;
}

function addStartDate(config, row) {
  var startDate = row[config.dateColumn];
...
function addColumnsWithRules(config, row) {
  ...
  ['Project', 'Client', 'Task', 'Tags'].forEach(function(column) {
    ...
    var columnId = column.toLowerCase();
    if (config[columnId + 'Column']) { // Use an specified column from source CSV
      newRow[column] = row[config[columnId + 'Column']];
    } else if (config.columns && config.columns[column]) { // or a defined set of rules
...
function addDescription(config, row) {
  ...
  if (config.descriptionColumn) {
    row['Description'] = row[config.descriptionColumn];
  }
  return row;
}
