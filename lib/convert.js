...
module.exports = function convertMite2Toggl(options, callback) {
  ...
  papaparseConfig.complete = function(parsedCsv) {
    var converted = parsedCsv.data
                      ...
                      .map(addRest.bind(null, options.config))
...
function addRest(config, row) {
  ...
  ['Project', 'Client', 'Task', 'Tags', 'Description'].forEach(function(column) {
    ...
    if (config.fields && config.fields[column]) {
      var sourceColumn = config.fields[column].column;
      if (sourceColumn) { // Use an specified column from source CSV
        var translations = config.fields[column].translations;
        newRow[column] = (translations && translations[row[sourceColumn]]) ?
                           translations[row[sourceColumn]] : row[sourceColumn];
      }

      var rules = config.fields[column].rules;
      if (rules) { // use a set of rules for taking data from other columns
